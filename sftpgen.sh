...
# Script must be ran as root because it creates users
if [[ $EUID -ne 0 ]]; then
...
# Print existing sftp users by iterating through /etc/passwd
...
if [ "$check_user" == "1" ] ; then
  ...
  creds="\n  Username: $ticketvar \n  Password: $userpass \n  Expiration: $expire\n\n"
  # Print the credentials of the new user. They are only printed on screen.
  # If credentials are forgotten you must reset the password to the user.
  printf "$creds"
